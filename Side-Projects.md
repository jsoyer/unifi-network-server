...
and use `jsoyer/unifi-network-server:beta` for the image
like this:
...
docker run -d --init \
   ...
   jsoyer/unifi-network-server:beta
```

### Running the Beta Using `docker-compose.yml` 

In the containers service definition of the `docker-compose.yml` file, replace `image: jsoyer/unifi-network-server` with the following:

```shell
        image: jsoyer/unifi-network-server:beta
         environment:
