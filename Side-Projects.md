...
and use `jsoyer/unifi:beta` for the image
like this:
...
In the containers service definition of the `docker-compose.yml` file, replace `image: jsoyer/unifi` with the following:

```shell
        image: jsoyer/unifi:beta
         environment:
